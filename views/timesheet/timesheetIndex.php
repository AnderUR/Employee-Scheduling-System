...
$data['header'] = array();
$data['header'][0] = array(0, webuser::view($thisUser->getUID(), 'Shifts', '#'));
$data['header'][1] = array(0, 'Employee'); //$data['header'][2] = array(0, 'Area');
$data['header'][2] = array(500, date('D m-d', strtotime($week[DayInWeek::Sunday])));
$data['header'][3] = array(300, date('D m-d', strtotime($week[DayInWeek::Monday])));
$data['header'][4] = array(300, date('D m-d', strtotime($week[DayInWeek::Tuesday])));
$data['header'][5] = array(300, date('D m-d', strtotime($week[DayInWeek::Wednesday])));
$data['header'][6] = array(300, date('D m-d', strtotime($week[DayInWeek::Thursday])));
$data['header'][7] = array(300, date('D m-d', strtotime($week[DayInWeek::Friday])));
$data['header'][8] = array(300, date('D m-d', strtotime($week[DayInWeek::Saturday])));
            ...
            <div class="large-10 medium-12 small-9 small-centered medium-centered columns">
                <div class="row">
                    ...
                    <div id="srchWeekContainer" class="large-5 small-4 columns">
                        <input readonly="true" id="srchWeekDate" type="search" value="Show Week Of..." />
                    </div>
                    ...
                    <div id="schSortContainer" class="small-4 columns">
                        <select onchange="if (this.value)
                            ...
                            <option value="<?= site_url('/timesheet/timesheetLabIndex/'.$week[DayInWeek::Sunday]); ?>">Area</option>
                        </select>
                    </div>

                    <div id="schTodayContainer" class="nowrap large-3 small-4 columns">
                      <?= anchor('/timesheet/timesheetIndex/'.date('Y-m-d'), 'This Week', 'id="todayButton" class="button"');?>
                    </div>
            ...
            <div class="row">
                ...
                <div class="large-7 medium-12 small-12 large-centered medium-centered columns">
                    ...
                    <div id="prevContainer" class="small-1 columns">
                        <a href="<?= site_url('/timesheet/timesheetIndex/'.$lastWeekDate->format('Y-m-d')); ?>">
                    ...
                    <div id="nextContainer" class="small-1 columns">
                        <a href="<?= site_url('/timesheet/timesheetIndex/'.$nextWeekDate->format('Y-m-d')); ?>">
